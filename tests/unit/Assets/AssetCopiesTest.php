<?php
/**
 * Tests for asset source resolution, optimized asset copies (naming, pipeline, queue,
 * budget, GC) and the tag rewriter.
 *
...
final class AssetCopiesTest extends TestCase {
	...
	protected function setUp(): void {
		shso_test_reset();
		wp_clear_scheduled_hook( AssetCopies::CRON_HOOK );
		AssetSource::reset();
		AssetRewriter::reset();
	...
	public function test_frontend_generates_small_files_synchronously_within_budget(): void {
		...
		$this->assertNotFalse( wp_next_scheduled( AssetCopies::CRON_HOOK ), 'The background worker is scheduled.' );
	...
	// ---------------------------------------------------------------------
	// Source resolution (AssetSource): URL → local file safety and attribution.
	// ---------------------------------------------------------------------

	/**
	 * Files used by the source resolution tests.
	 */
	private function source_fixtures(): void {
		AssetSource::reset();
	...
	public function test_source_resolves_plugin_stylesheet_with_query_string(): void {
		$this->source_fixtures();
	...
	public function test_source_protocol_relative_and_root_relative_urls(): void {
		$this->source_fixtures();
	...
	public function test_source_rejects_other_hosts_and_schemes(): void {
		$this->source_fixtures();
	...
	public function test_source_rejects_traversal(): void {
		$this->source_fixtures();
	...
	public function test_source_rejects_non_asset_extensions_and_missing_files(): void {
		$this->source_fixtures();
	...
	public function test_source_rejects_symlink_escaping_the_roots(): void {
		$this->source_fixtures();
	...
	public function test_source_symlink_inside_roots_is_allowed_but_extension_of_target_counts(): void {
		$this->source_fixtures();
	...
	public function test_source_rejects_files_over_the_size_limit(): void {
		$this->source_fixtures();
	...
	public function test_source_minified_detection(): void {
		$this->source_fixtures();
	...
	public function test_source_attribution(): void {
		$this->source_fixtures();
		$cases = array(
	...
	public function test_source_is_local_url(): void {
		$this->source_fixtures();
	...
	public function test_source_resolve_in_with_custom_environment(): void {
		$this->source_fixtures();
	...
	public function test_pipeline_registers_cron_hook_for_deactivation(): void {
		$hooks = \SH\SpeedOptimizer\Modules\AssetOptimization\AssetPipeline::cron_hooks( array( 'shso_cron_daily' ) );
		$this->assertContains( AssetCopies::CRON_HOOK, $hooks );
		$this->assertContains( AssetCopies::CRON_HOOK, \SH\SpeedOptimizer\Modules\AssetOptimization\AssetPipeline::cron_hooks( null ) );
	}
